...
interface ShippingCalculatorSectionProps {
  title?: string
  description?: string
  className?: string
}
  ...
  const calculateRates = async () => {
    ...
    try {
      ...
      if (!data.rates || data.rates.length === 0) {
        setError(
          data.message || "No shipping rates available for this route. Please check your ZIP codes and try again.",
        )
      } else {
        toast({
          title: "Rates Updated",
        ...
        <Card className="w-full max-w-4xl mx-auto">
          <CardHeader className="text-center">
            <CardTitle className="flex items-center justify-center gap-2 text-lg sm:text-xl">
              ...
              {dataSource === "shippo" && (
                <CheckCircle className="h-4 w-4 text-green-600" title="Live data from Shippo" />
              )}
            </CardTitle>
            <CardDescription className="text-sm sm:text-base">
              Enter your shipping details to get real-time rates
            </CardDescription>
          </CardHeader>
            ...
            {rates.length > 0 && (
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-base sm:text-lg font-semibold">Available Shipping Options</h3>
                  {dataSource === "shippo" && (

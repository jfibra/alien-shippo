...
export default async function ConfirmEmailChangePage({
  ...
  let message = "Confirming your email change..."
  let isSuccess = false
  let error = null
  ...
  if (token_hash && type) {
    const { error: verifyError } = await supabase.auth.verifyOtp({
      token_hash,
      type: type === "email_change" ? "email_change" : "signup", // Ensure type is correct
    })
    ...
    if (!verifyError) {
      ...
      message = `Failed to confirm email change: ${error}`
    }
  } else {
    error = "Invalid token or type provided."
    message = "Invalid link for email change confirmation."
  }
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className={isSuccess ? "text-green-600" : "text-red-600"}>
            {isSuccess ? "Email Change Confirmed!" : "Confirmation Failed"}
          </CardTitle>

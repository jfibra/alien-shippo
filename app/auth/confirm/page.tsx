...
export default async function ConfirmPage({
  ...
  let message = "Confirming your account..."
  let isSuccess = false
  let error = null
  ...
  if (token_hash && type) {
    const { error: verifyError } = await supabase.auth.verifyOtp({
      token_hash,
      type: type === "signup" ? "signup" : "email_change", // Ensure type is correct
    })
    ...
    if (!verifyError) {
      ...
      message = `Failed to confirm account: ${error}`
    }
  } else {
    error = "Invalid token or type provided."
    message = "Invalid link for account confirmation."
  }
